...
export const ticketCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "open") {
      ...
      const [ticket] = await db.insert(ticketsTable).values({ guildId, ticketNumber: counter, userId: interaction.user.id, userTag: interaction.user.tag, subject, channelId: channel.id }).returning();

      const embed = new EmbedBuilder().setTitle(`🎫 Ticket #${counter}`).setColor(0x5865f2)
        .setDescription(`Thank you for opening a ticket, <@${interaction.user.id}>!\n\n**Subject:** ${subject}\n\nOur staff will be with you shortly.`)
        .setTimestamp();
