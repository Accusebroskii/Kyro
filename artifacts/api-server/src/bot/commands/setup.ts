...
export const setupCommand = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s.setName("modmail")
        .setDescription("Configure ModMail forum channel")
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === "welcome") {
      ...
      await db.update(guildConfigTable).set({ ...(cat ? { ticketCategoryId: cat.id } : {}), ...(log ? { ticketLogChannelId: log.id } : {}) }).where(eq(guildConfigTable.guildId, guildId));
      ...
      await db.update(guildConfigTable).set({ ...(antispam !== null ? { antispamEnabled: antispam } : {}), ...(antiraid !== null ? { antiRaidEnabled: antiraid } : {}), ...(automod !== null ? { automodEnabled: automod } : {}) }).where(eq(guildConfigTable.guildId, guildId));
      ...
      await db.update(guildConfigTable).set({ ...(mod ? { modRoleId: mod.id } : {}), ...(admin ? { adminRoleId: admin.id } : {}), ...(mute ? { muteRoleId: mute.id } : {}) }).where(eq(guildConfigTable.guildId, guildId));
      ...
      const embed = new EmbedBuilder().setTitle("⚙️ Bot Configuration").setColor(0x5865f2)
        .addFields(
